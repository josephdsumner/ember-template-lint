...
function hasAttributeValue(node, attributeName, nodeAttributeValue, unwrapTextNode = false) {
  const attributeNode = findAttribute(node, attributeName);
  if (!attributeNode) {
    return false;
  }
  return attributeValue(attributeNode, unwrapTextNode) === nodeAttributeValue;
}

function elementAttributeValue(node, attributeName, unwrapTextNode = false) {
  return attributeValue(findAttribute(node, attributeName), unwrapTextNode);
}
...
function attributeValue(attrNode, unwrapTextNode = false) {
  if (!attrNode) {
    return;
  }
  if (attrNode.value && attrNode.value.type === 'TextNode' && unwrapTextNode) {
