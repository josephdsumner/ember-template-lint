...
function hasButtonParent(path) {
  ...
  let buttonRoleInParentPath = parents.find(
    (parent) => {
      let parentRoleAttrNode = AstNodeInfo.findAttribute(parent.node, 'role');
      let parentRoleAttrValue;
      if (parentRoleAttrNode) {
        if(parentRoleAttrNode.value && parentRoleAttrNode.value.type === 'TextNode') {
          parentRoleAttrValue = parentRoleAttrNode.value.chars;
        } else {
          parentRoleAttrValue = parentRoleAttrNode.value;
        }
      }
      return parent.node.type === 'ElementNode' && parentRoleAttrValue === 'button';
    }
  );
