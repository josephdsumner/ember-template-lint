...
module.exports = class NoDuplicateId extends Rule {
  ...
  // Helper for getting `id` attribute nodes from parent AST Node types that
  // store their attributes as an Array of HashPairs, e.g.:
  // - MustacheStatement: `id`
  // - BlockStatement: `elementId`
  getIdHashArgIdResult(node, idAttrName) {
    let refPair = { key: idAttrName, value: { type: 'StringLiteral' } };
    return node.hash.pairs.find((testPair) => NodeMatcher.match(testPair, refPair));
  }
  ...
  visitor() {
    ...
    return {
      ...
      BlockStatement(node) {
        let elementIdHashArg = this.getIdHashArgIdResult(node, 'elementId');
        if (!elementIdHashArg) {
          return;
        }
      ...
      MustacheStatement(node) {
        let idHashArg = this.getIdHashArgIdResult(node, 'id');
        if (!idHashArg) {
          return;
        }
