...
module.exports = class NoDuplicateId extends Rule {
  ...
  // It receives the appropriate name for the parent node's `id` attribute
  // (i.e., `id` for MustacheStatements and `elementId` for BlockStatements)
  // from the visitor function. The name is inserted into a template HashPair
  // object, which then forms the refNode for a `match`-based lookup.
  // If a valid `id` attribute HashPair is found, then the attribute *value*
  // will be returned to the visitor so it can be duplicate-checked against the
  // existing Set of idAttrValues. Otherwise, the default return -- undefined --
  // triggers visitor return / move to the next node.
